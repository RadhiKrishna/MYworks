      ...
      <div className="relative min-h-[420px]">
        ...
        <div className="relative max-w-content mx-auto px-6 md:px-10 py-20 md:py-24 grid md:grid-cols-[1.1fr_0.9fr] gap-12 items-center">
          <Reveal>
            ...
            <Button href="/district-cooling" className="mt-7">
          ...
          <div className="grid grid-cols-3 gap-4">
            {districtCoolingStats.map((s, i) => (
              <Reveal key={s.label} delay={i * 100}>
                <div className="rounded-lg bg-ink/50 border border-paper/10 backdrop-blur-sm px-4 py-5 text-center hover:border-gold/40 transition-colors">
                  <p className="font-display text-3xl text-gold">{s.value}</p>
                  <p className="text-paper/70 text-xs mt-1.5 leading-snug">
                    {s.label}
                  </p>
                </div>
              </Reveal>
